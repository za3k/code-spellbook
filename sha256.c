...
L b,l;I f[64],s[8]={0x6a09e667,0xbb67ae85,0x3c6ef372,0xa54ff53a,0x510e527f,0x9\
...
506ceb,0xbef9a3f7,0xc67178f2};//2nd, 3rd root of 1-64th primes, fractional part
I R(I a,I b){return a>>b|a<<(32-b);}I S(I v,I a,I b,I c){return R(v,a)^R(v,b)^(v
>>c);}I E(I v,I a,I b,I c){return R(v,a)^R(v,b)^R(v,c);}//updates every 64C
void U(C i){I a,B,c,d,e,F,g,h,u,v,j,m[64];  f[b/32%16]|=i<<8*(3-b/8%4);b+=8;
if(b%512==0){a=s[0];B=s[1];c=s[2];d=s[3];e=s[4];F=s[5];g=s[6];h=s[7];
for(j=0;j<16;++j){m[j]=f[j];f[j]=0;}
for(;j<64;++j)m[j]=S(m[j-2],17,19,10)+m[j-7]+S(m[j-15],7,18,3)+m[j-16];
for(j=0;j<64;++j){u=h+E(e,6,11,25)+(e&F^~e&g)+k[j]+m[j];v=E(a,2,13,22)+(a&B^a&c^
B&c);h=g;g=F;F=e;e=d+u;d=c;c=B;B=a;a=u+v;}s[0]+=a;s[1]+=B;s[2]+=c;s[3]+=d;s[4]+=
e;s[5]+=F;s[6]+=g;s[7]+=h;}}
int main(){I i,j,c;while((c=getchar())!=EOF)U(c);
l=b;U(0x80);while(b%512!=448)U(0);for(i=0;i<8;++i)U(l>>8*(7-i));
for(i=0;i<64;++i)putchar("0123456789abcdef"[(s[i/8]>>(7-i%8)*4)&0xf]);}
